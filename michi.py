...
PROB_SSAREJECT = (
    0.9  # probability of rejecting suggested self-atari in playout
)
...
pat_gridcular_seq = [  # Sequence of coordinate offsets of progressively wider diameters in gridcular metric
    [
        ...
        [1, -1],
        [-1, -1],
    ],  # d=1,2 is not considered separately
    [
        [0, 2],
        [0, -2],
        [2, 0],
...
class Position:
    ...
    def move(self, c):
        ...
        for d in neighbors(c):
            ...
            fboard = floodfill(
                board, d
            )  # get a board with the adjecent group replaced by '#'
...
def empty_position():
    ...
    return Position(
        board=empty, cap=(0, 0), n=0, ko=None, last=None, last2=None, komi=6.5
    )
...
def fix_atari(
    pos, c, singlept_ok=False, twolib_test=True, twolib_edgeonly=False
):
    ...
    if l_new_2 is not None:
        ...
        # escape routes!)
        if solutions or not (
            contact(board_put(fboard, l_new_2, 'L'), '.') is None
...
def empty_area(board, c, dist=3):
    ...
    for d in neighbors(c):
        if board[d] in 'Xx':
            ...
            board[d] == '.'
            and dist > 1
            and not empty_area(board, d, dist - 1)
        ):
            return False
    return True
...
def pat3_expand(pat):
    ...
    def pat_wildexp(p, c, to):
        ...
        return reduce(
            lambda a, b: a + b,
            [pat_wildexp(p[:i] + t + p[i + 1 :], c, to) for t in to],
        )
...
def gen_playout_moves(
    pos, heuristic_set, probs={'capture': 1, 'pat3': 1}, expensive_ok=False
):
    ...
    if random.random() <= probs['pat3']:
        ...
        for c in heuristic_set:
            if (
                pos.board[c] == '.'
                and c not in already_suggested
                and neighborhood_33(pos.board, c) in pat3set
            ):
                yield (c, 'pat3')
    ...
    try:
        ...
        while passes < 2 and pos.n < MAX_GAME_LEN:
            ...
            for c, kind in gen_playout_moves(
                pos, pos.last_moves_neighbors(), PROB_HEURISTIC
            ):
                if disp and kind != 'random':
                    print(
                        'move suggestion', str_coord(c), kind, file=sys.stderr
                    )
                pos2 = pos.move(c)
                if pos2 is None:
                    continue
                ...
                if random.random() <= (
                    ...
                    in_atari, ds = fix_atari(
                        pos2, c, singlept_ok=True, twolib_edgeonly=True
                    )
                    if ds:
                        if disp:
                            print(
                                'rejecting self-atari move',
                                str_coord(c),
                                file=sys.stderr,
                            )
                        pos2 = None
        ...
        if disp:
            print(
                '** SCORE B%+.1f **' % (score if pos.n % 2 == 0 else -score),
                file=sys.stderr,
            )
...
class TreeNode:
    ...
    def expand(self):
        ...
        cfg_map = (
            cfg_distances(self.pos.board, self.pos.last)
        ...
        for c, kind in gen_playout_moves(
            self.pos, range(N, (N + 1) * W), expensive_ok=True
        ):
            ...
            if kind.startswith('capture'):
                ...
                if (
                    floodfill(self.pos.board, int(kind.split()[1])).count('#')
                    > 1
                ):
    ...
    def best_move(self):
        ...
        return (
            max(self.children, key=lambda node: node.v)
...
def tree_descend(tree, amaf_map, disp=False):
    ...
    while nodes[-1].children is not None and passes < 2:
        ...
        if disp:
            print('chosen %s' % (str_coord(node.pos.last),), file=sys.stderr)
        if node.pos.last is None:
            passes += 1
        else:
            passes = 0
            if (
                amaf_map[node.pos.last] == 0
            ):  # Mark the coordinate with 1 for black
                amaf_map[node.pos.last] = (
                    1 if nodes[-2].pos.n % 2 == 0 else -1
                )
...
def tree_update(nodes, amaf_map, score, disp=False):
    ...
    for node in reversed(nodes):
        ...
        node.w += (
            score < 0
        )  # score is for to-play, node statistics for just-played
...
def tree_search(tree, n, owner_map, disp=False):
    ...
    while i < n:
        ...
        if (
            i > n * 0.05
            and best_wr > FASTPLAY5_THRES
            or i > n * 0.2
            and best_wr > FASTPLAY20_THRES
        ):
            break
...
def print_pos(pos, f=sys.stderr, owner_map=None):
    ...
    if pos.n % 2 == 0:  # to-play is black
        ...
        Ocap, Xcap = pos.cap
    print(
        'Move: %-3d   Black: %d caps   White: %d caps  Komi: %.1f'
    ...
    if pos.last is not None:
        pretty_board = (
            pretty_board[: pos.last * 2 - 1]
            + '('
            + board[pos.last]
            + ')'
            + pretty_board[pos.last * 2 + 2 :]
        )
    rowcounter = count()
    pretty_board = [
        ' %-02d%s' % (N - i, row[2:])
        for row, i in zip(pretty_board.split("\n")[1:], rowcounter)
    ]
    if owner_map is not None:
        pretty_ownermap = ''
        for c in range(W * W):
        ...
        pretty_board = [
            '%s   %s' % (brow, orow[2:])
            for brow, orow in zip(
                pretty_board, pretty_ownermap.split("\n")[1:]
            )
        ]
...
def dump_subtree(
    node, thres=N_SIMS / 50, indent=0, f=sys.stderr, recurse=True
):
    """print this node and all its children with v >= thres."""
    print(
        "%s+- %s %.3f (%d/%d, prior %d/%d, rave %d/%d=%.3f, urgency %.3f)"
        % (
            indent * ' ',
            str_coord(node.pos.last),
            node.winrate(),
...
def print_tree_summary(tree, sims, f=sys.stderr):
    ...
    while node is not None:
        ...
        node = node.best_move()
    print(
        '[%4d] winrate %.3f | seq %s | can %s'
        % (
            sims,
            best_nodes[0].winrate(),
            ' '.join([str_coord(c) for c in best_seq[1:6]]),
            ' '.join(
                [
                    '%s(%.3f)' % (str_coord(n.pos.last), n.winrate())
                    for n in best_nodes
                ]
            ),
        ),
...
def gtp_io():
    ...
    while True:
        ...
        if command[0] == "boardsize":
            ...
            tree.pos = Position(
                board=tree.pos.board,
                cap=(tree.pos.cap[0], tree.pos.cap[1]),
                n=tree.pos.n,
                ko=tree.pos.ko,
                last=tree.pos.last,
                last2=tree.pos.last2,
            ...
            if c is not None:
                # Find the next node in the game tree and proceed there
                if tree.children is not None and filter(
                    lambda n: n.pos.last == c, tree.children
                ):
            ...
            print(
                'Warning: Ignoring unknown command - %s' % (line,),
                file=sys.stderr,
            )
            ret = None
